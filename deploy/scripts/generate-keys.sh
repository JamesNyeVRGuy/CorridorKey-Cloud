# This script is taken directly from Supabases official self-hosting documentation.
# Original source: https://github.com/supabase/supabase/blob/master/docker/utils/generate-keys.sh
#
#!/bin/sh
#
...
echo "CK_SUPABASE_SERVICE_KEY=${service_role_key}"
echo ""
...
sed -i.old \
    ...
    -e "s|^CK_JWT_SECRET=.*$|CK_JWT_SECRET=${jwt_secret}|" \
    ...
    -e "s|^CK_SUPABASE_SERVICE_KEY=.*$|CK_SUPABASE_SERVICE_KEY=${service_role_key}|" \
    ...
    -e "s|_POSTGRES_PASSWORD_|${postgres_password}|g" \
    .env